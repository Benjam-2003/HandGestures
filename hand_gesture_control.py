...
import pyautogui
import time
import RPIservo # Import library to control servos
...
def detect_gestures(landmarks):
    ...
    if (
        ...
        sc.singleServo(13,-1,2)
        time.sleep(1)
        sc.stopWiggle()
        return time.time()

    ################# Index finger bend down gesture detection to close clamp
    # Calculate angle between finger and palm using trigonometry
    dx = index_tip.x - index_base.x
    dy = index_tip.y - index_base.y
    angle = np.arctan2(dy, dx) * 180 / np.pi

    # Check if finger angle exceeds threshold to close clamp
    if angle > threshold_angle:
        sc.singleServo(15,1,2)
        time.sleep(1)
        sc.stopWiggle()
        return time.time()
    ...
    # Check if finger angle exceeds threshold to open clasp
    if angle > threshold_angle:
        sc.singleServo(15,-1,2)
        time.sleep(1)
        sc.stopWiggle()
    ...
    # Calculate angle between ring finger and palm using trigonometry
    dx = ring_tip.x - middle_base.x
    dy = ring_tip.y - middle_base.y
    angle = np.arctan2(dy, dx) * 180 / np.pi

    # Check if finger angle exceeds threshold to move motor 12 forward
    if angle > threshold_angle:
        sc.singleServo(12,-1,2)
        time.sleep(1)
        sc.stopWiggle()
        return time.time()
    
    # Calculate angle between pinky finger and palm using trigonometry
    dx = pinky_tip.x - middle_base.x
    dy = pinky_tip.y - middle_base.y
    angle = np.arctan2(dy, dx) * 180 / np.pi

    # Check if finger angle exceeds threshold to move motor 12 backward
    if angle > threshold_angle:
        sc.singleServo(12,1,2)
        time.sleep(1)
        sc.stopWiggle()
        ...
        # Calculate angle between thumb tip and palm using trigonometry
    dx = thumb_tip.x - middle_base.x
    dy = thumb_tip.y - middle_base.y
    angle = np.arctan2(dy, dx) * 180 / np.pi

    # Check if finger angle exceeds threshold to move motor 13 backward
    if angle > threshold_angle:
        sc.singleServo(13,1,2)
        time.sleep(1)
        sc.stopWiggle()
